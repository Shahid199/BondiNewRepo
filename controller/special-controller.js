...
const createSpecialExam = async (req, res, next) => {
  ...
  let saveExam = new SpecialExam({
    ...
    totalMarksWritten: totalMarksWritten,
    totalMarks: totalMarks,
    noOfTotalSubject: Number(noOfTotalSubject),
    noOfExamSubject: Number(noOfExamSubject),
    noOfOptionalSubject: Number(noOfOptionalSubject),
    noOfFixedSubject: Number(noOfFixedSubject),
    subjectInfo: subjectsInfos,
...
const assignQuestionMcq = async (req, res, next) => {
  ...
  const examType = Number(JSON.parse(req.query.type));
