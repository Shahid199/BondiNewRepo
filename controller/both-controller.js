...
const bothGetMcqQuestionByExamId = async (req, res, next) => {
  ...
  let questions = null
  try {
    questions = await BothMcqQuestionVsExam.findOne({ eId: examId }, 'mId -_id')
  } catch (err) {
    return res.status(500).json('Something went wrong.')
  }
...
const updateBothStudentMarks = async (req, res, next) => {
  ...
  let updStatus = null
  try {
    updStatus = await Exam.BothStudentExamVsQuestions.updateOne({ _id: new mongoose.Types.ObjectId("66611a9aa45a1de46a09fc3e") }, getData)
  } catch (err) {
    return res.status(500).json(err)
  }
